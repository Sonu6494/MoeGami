...
// Valid AniList statuses that map to our EntryStatus union
const VALID_STATUSES = new Set<EntryStatus>([
  "COMPLETED", "CURRENT", "PAUSED", "DROPPED", "PLANNING", "REPEATING",
])

function toEntryStatus(raw: string): EntryStatus {
  if (VALID_STATUSES.has(raw as EntryStatus)) return raw as EntryStatus
  return "PLANNING"
}
...
function normalise(
  ...
  return {
    ...
    episodes_watched: listEntry.progress,
    user_completed: listEntry.status === "COMPLETED",
    status: toEntryStatus(listEntry.status),
...
export async function fetchAndNormalise(
  ...
  // Deduplicate across lists by platform_id — same entry can appear in
  // multiple AniList lists (e.g. Completed + Rewatching)
  const seen = new Set<number>()
  const entries: NormalisedEntry[] = []

  for (const list of collection.lists) {
    for (const listEntry of list.entries) {
      if (!listEntry.media) continue
      if (seen.has(listEntry.media.id)) continue
      seen.add(listEntry.media.id)
